...
export function useProjectHydration({
  ...
  const refresh = async () => {
    ...
    try {
      ...
      // edges
      // edges (inside useProjectHydration.refresh)
        const { data: edgesData = [], error: edgesErr } = await supabase
        .from("edges")
        .select("id, source_node, target_node")
        .eq("project_id", projectId)
        // optional: .is("deleted_at", null) if you soft-delete
        ;
        ...
        // IMPORTANT: add server edge with id and do NOT emit onEdgeCreate
        nodeCanvasRef.current.addEdge?.({
        id: eRow.id,
        sourceId: eRow.source_node,
        targetId: eRow.target_node,
        emit: false, 
        });
        }
